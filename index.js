...
function addGamesToPage(games) {
    ...
    for (const game of games) {
        ...
        // about each game
        newGameCard.innerHTML = `<img src = "${game.img}" class="game-img"> 
                                 <h3>${game.name}</h3>
    ...
    // To get the secret key component 1, print the length of games
    console.log("Secret key compoent 1:", games.length);
}
...
function filterUnfundedOnly() {
    // Clear all the currently displayed games
...
`A total of $${totalRaised.toLocaleString('en-US')} has been raised for ${GAMES_JSON.length}
 ${GAMES_JSON.length > 1 ? 'games' : 'game'}. Currently, ${unfundedCount} 
 ${unfundedCount > 1 ? 'games remain': 'game remains'} unfuded. We need your help to fund these
 amazing games!`;
